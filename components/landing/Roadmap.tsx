...
export default function Roadmap({ title, items }: RoadmapProps) {
  return (
    <section aria-labelledby="roadmap-heading" className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
      <div className="space-y-8">
        <div id="roadmap-heading" className="text-center md:text-left">
          <LanguageText value={title} as="h2" className="font-display text-3xl tracking-tight text-ink sm:text-4xl" />
        </div>
        <div className="space-y-4 md:relative md:border-l md:border-black/5 md:pl-10">
          {items.map((item, index) => {
            const Icon = timelineIcons[index] ?? Satellite;
            return (
              <article key={item.es} className="timeline-card relative">
